...
test("o cache-bust acompanhou a última mudança do app.js", () => {
  ...
  assert.equal(versao, "20260925-quantidade-fracionada");
  ...
  assert.match(app, new RegExp(`from "\\./js/services/pdv-order-alerts\\.js\\?v=${versao}"`));
  // O núcleo de assinatura também é módulo importado: segue a mesma versão do app.js
  assert.match(app, new RegExp(`from "\\./js/ui/assinatura\\.js\\?v=${versao}"`));
});
