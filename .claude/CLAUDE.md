...
- **Credenciais do banco**: via `DATABASE_URL` no `.env.local`. Existe um fallback legado em `.streamlit/secrets.toml` (`server/db.js`, função `readStreamlitSecret`) usado quando `DATABASE_URL` está vazia — resquício de uma versão anterior em Streamlit/Python, e **ele aponta para o Supabase antigo**. Se algo parecer estar lendo/gravando dados errados, a primeira suspeita é `DATABASE_URL` não ter sido carregada e a conexão ter caído nesse fallback.
...
- **Nunca deixe variáveis da Vercel no `.env.local`** (`VERCEL`, `VERCEL_*`, `TURBO_*`, `NX_DAEMON`), como as que vêm de `vercel env pull`. Com `VERCEL` definida, o `server/index.js` acha que está em ambiente serverless, **pula o `listen()` e encerra na hora** (servidor não sobe), além de forçar cookie `secure`, que quebra o login em HTTP na LAN.

## Deploy

- **Produção atual: rede local (LAN)**, não Vercel. Veja [DEPLOY_LOCAL.md](DEPLOY_LOCAL.md). O servidor roda como serviço do Windows (NSSM, nome do serviço `MyEstoque`) — sempre ativo, inicia sozinho no boot, ninguém precisa iniciar nada manualmente. Banco é PostgreSQL local (usuário `myestoque_app`), não mais Supabase.
- Vercel (`vercel.json`, `api/index.js`) foi abandonado como hospedagem principal depois que o plano gratuito bloqueou o deploy por limite de uso — os arquivos continuam no repo como opção secundária, mas não são o caminho ativo. Veja [DEPLOY.md](DEPLOY.md) só se for reativar isso.
- `NODE_ENV=production` é seguro no local: o cookie de sessão só fica `secure` se `VERCEL` estiver setado ou `FORCE_SECURE_COOKIES=true` — não depende mais de `NODE_ENV` (isso foi corrigido de propósito para não quebrar login em HTTP puro na LAN).
...
- `tests/supabase-hardening.test.js` varre o repo procurando segredos vazados — cuidado ao criar pastas com `node_modules` symlinkado dentro do repo (já causou um bug de crash, resolvido removendo a pasta problemática).
