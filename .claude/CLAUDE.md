...
- **Fator de conversão em lote, nunca por linha**: `obterFatoresEmLote()` busca `product_integration_mappings` com `sku_produto = ANY($1)` e desempata em JS. Testado (e descartado) um `LEFT JOIN LATERAL` por linha com índice dedicado: 98ms para 60 SKUs, porque o planejador preferia varredura por causa do `ORDER BY` externo. A versão em lote com índice `idx_mappings_sku_ativo (sku_produto) WHERE active` custa 1,6ms (era 32,3ms sem o índice — 20x). **Esse índice foi criado direto em produção e depois replicado para `db/estrutura.dump`** via `npm run dump:gerar` — sem isso, toda instalação nova nasceria sem ele e a consulta voltaria a ser 20x mais lenta silenciosamente.
