...
- **Produção atual: rede local (LAN)**, não Vercel. Veja [DEPLOY_LOCAL.md](../docs/DEPLOY_LOCAL.md). O servidor roda como serviço do Windows (NSSM, nome do serviço `MyEstoque`) — sempre ativo, inicia sozinho no boot, ninguém precisa iniciar nada manualmente. Banco é PostgreSQL local (usuário `myestoque_app`).
- Vercel (`vercel.json`, `api/index.js`) foi abandonado como hospedagem principal depois que o plano gratuito bloqueou o deploy por limite de uso — os arquivos continuam no repo como opção secundária, mas não são o caminho ativo. Veja [DEPLOY.md](../docs/DEPLOY.md) só se for reativar isso.
